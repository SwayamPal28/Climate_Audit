#!/usr/bin/env python3
"""
Add India data by copying existing row structure
"""
...
for filename in sector_files.values():
    ...
    clean_lines = [line for line in lines if 'IND' not in line[-20:]]
    
    with open(filepath, 'w') as f:
        f.writelines(clean_lines)

print("✅ Cleaned old India data")

# Add new data using proper CSV format
total = 0
for sector, edges in DIVERSE_INDIA_DATA.items():
    filepath = data_dir / sector_files[sector]
    
    # Read to get a template row and header
    with open(filepath, 'r') as f:
        reader = csv.reader(f)
        header = next(reader)
        template_row = next(reader)  # Get first data row as template
    ...
    with open(filepath, 'a', newline='') as f:
        ...
        for partner, direction, value_b in edges:
            ...
            # Copy template and modify key fields
            row = template_row.copy()
            row[43] = str(value)  # primaryValue
            row[49] = src  # src_iso  
            row[50] = tgt  # tgt_iso
...
print(f"✅ Added {total} India edges with proper CSV format")
print("🔄 Reloading...")
