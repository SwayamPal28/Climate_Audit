...
class GNNWrapper:
    def __init__(self, device='cpu'):
        ...
        if os.path.exists(nodes_path):
            ...
            self.encoder.fit(train_nodes[['wb_code']])  # Fit only once during initialization
    ...
    def _encode_features(self, df):
        ...
        available_numeric_cols = [col for col in expected_numeric_cols if col in df.columns]
        ...
        try:
            ...
            print(f"Problematic values in data: {df[available_numeric_cols].loc[df[available_numeric_cols].isna().any(axis=1)]}")
            raise
    ...
    # Handle edge_index
        if edge_index is None:
            num_nodes = len(df)
            edge_index = torch.combinations(torch.arange(num_nodes, device=self.device)).t()
        else:
