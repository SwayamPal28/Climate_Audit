...
"""
    ...
    def _build_diplomatic_prompt(self, turn_data: Dict) -> str:
        ...
        prompt = f"""You are a game theory expert analyzing a climate policy negotiation.

SCENARIO: {player} imposed a carbon tariff on {rival}. The AI (playing as {rival}) retaliated.

PLAYER ACTION ({player}):
- Action Type: {player_action.get('action_type', 'TARIFF')}
- Sector Targeted: {player_action.get('sector', 'Unknown')}
- Severity: {player_action.get('severity', 0) * 100:.1f}%
- Damage Inflicted: ${player_action.get('damage_inflicted', 0) / 1e6:.1f}M

AI RESPONSE ({rival}):
- AI Persona: {ai_persona}
- Retaliation: {ai_reaction.get('action', 'Unknown')}
- Counter-Tariff: {ai_reaction.get('tariff_rate', 0) * 100:.1f}%
- Counter-Damage: ${ai_reaction.get('estimated_damage_to_opponent', 0) / 1e6:.1f}M
- Description: {ai_reaction.get('description', 'No description')}

Please analyze:

## Strategic Analysis
[Why did the AI choose this specific response?]

## Why Retaliated
[What triggered the retaliation? Was it proportional?]

## Next Move
[What should {player} do next to reach equilibrium?]

Frame this as a chess match. Keep under 350 words.
"""
        return prompt
