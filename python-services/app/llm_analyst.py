...
class LLMAnalystEngine:
    ...
    def __init__(self):
        ...
        if not api_key or api_key == 'your_actual_api_key_here' or "todo" in api_key.lower():
            ...
            try:
                # Configure Gemini client only if we have a valid key
        ...
        if self.api_key_valid:
            ...
            print(f"✅ LLM Analyst Engine initialized in Rule-Based mode (API key unavailable)")
    ...
    def analyze_policy_simulation(self, policy_data: Dict) -> Dict:
        ...
        if not self.api_key_valid or self.client is None:
            ...
            return {
                'executive_summary': f"Rule-based analysis: {policy_type} policy at {severity*100:.0f}% severity has been simulated.",
                'key_findings': f"Metrics computed: {', '.join(metrics.keys())}",
                'tradeoffs': "Full LLM analysis requires GEMINI_API_KEY configuration.",
                'recommendation': "Configure API key in .env file for detailed insights.",
    ...
    def analyze_shapley_attribution(self, shapley_data: Dict) -> Dict:
        ...
        if not self.api_key_valid or self.client is None:
            ...
            top_contributors = sorted(allocations.items(), key=lambda x: x[1], reverse=True)[:3]
            contrib_text = ", ".join([f"{c[0]} ({c[1]:.1f}%)" for c in top_contributors])
            
            return {
                'explanation': f"Rule-based analysis: {target}'s carbon responsibility of {total_co2:.1f} kt CO2 is attributed to: {contrib_text}. Configure GEMINI_API_KEY for detailed insights.",
                'target_country': target,
                'methodology': 'Shapley Value (Game Theory) - Rule-Based Fallback'
            }
